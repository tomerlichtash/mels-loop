...
describe('MLUtils', () => {
	describe('flattenArray', () => {
		...
		test('keep drop empty array as array', () => {
			expect(flattenArray(['a', [], 1, []])).toEqual(['a', 1]);
		});

		test('flatten comlpex nested arrays', () => {
			expect(
				flattenArray([
					[
						['a', 1],
	...
	describe('unique', () => {
		...
		test('generate unique ID with default key', () => {
			const id1 = unique.id();
			expect(id1.includes('mlid-')).toBeTruthy();
		});

		test('generate unique ID with custom key', () => {
