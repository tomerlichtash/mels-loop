...
import type { ProcessedContent, StoryConfig, ArticleMeta, Locale } from "../types";
import {
  fileExists,
  loadMarkdownFile,
  contentPath,
...
export async function getDocumentMeta(
  ...
  const config = await getStoryConfig(storySlug);
  return readSlugsMetadata(storySlug, "documents", config.documents ?? [], locale);
}
...
export async function getStoryDocuments(
  storySlug: string
): Promise<string[]> {
...
async function readSlugsMetadata(
  ...
  for (const slug of slugs) {
    const filePath = contentPath("stories", storySlug, folder, slug, localeFileName(locale));
    if (!(await fileExists(filePath))) {
