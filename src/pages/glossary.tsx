				...
				(<ul> {
					metaData.map((page, index) => {
						...
						return term && displayName?
							<li key={key} ><Link href={page.path} data-term-key={term.glossary_key}>{displayName}</Link></li>
							: <div key={key} className={classes.error}>Missing glossary term data {page.id}</div>
					})
				}
				</ul>)
			}
...
export const getStaticProps: GetStaticProps = async (context) => {
	...
	const childrenProps = mlNextUtils.getFolderStaticProps(CONTENT_TYPES.GLOSSARY, context,
		LoadFolderModes.CHILDREN, LoadContentModes.METADATA);
