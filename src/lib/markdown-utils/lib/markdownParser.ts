...
import { CaseInsensitiveMap } from 'lib/caseInsensitiveCollections';
import type {
...
class MarkdownParser implements IMarkdownParser {
	...
	public processParseTree(
		...
		const ctx = new ParseContext(mode, metaData);
		const ids = new CaseInsensitiveMap<IParsedNode>();
	...
	private processListNode(node: ParsedNode, context: ParseContext): IParsedNode {
		const resultNode: IParsedNode = {
	 ...
	 * @returns
	 */
	private processLinkDefinition(node: ParsedNode, context: ParseContext): IParsedNode {
	...
	private processOneASTNode(node: ParsedNode, context: ParseContext): IParsedNode {
	...
	private processAnchorLinkText(
		...
		const text = tmpl
			? mdUtils.translateString(tmpl.replace(INDEX_RE, String(target.sequence)))
	 ...
	 * @param context
	 */
	private tryProcessFigure(node: IParsedNode, context: ParseContext): void {
	...
	private promoteInlinesInNode(node: IParsedNode, context: ParseContext): void {
		const children = node.children;
	 ...
	 * @param context
	 */
	private promoteParagraphContent(node: IParsedNode, context: ParseContext): void {
		void context; // prevent warning, maintain general function signature
		const children = node.children;
		const newChildren: IParsedNode[] = this.collectInlines(node);

		children.length = 0;
		children.push(...newChildren);
	}
	// promoteParagraphContent
	private collectInlines(node: IParsedNode): IParsedNode[] {
		const inlines: IParsedNode[] = [];
	...
	private parsedNodeToMLNode(node: ParsedNode, context: ParseContext): IParsedNode {
		if (this.isIgnoredASTNode(node)) {
			return null;
		}
		...
		let currentLine: IParsedNode = parseMode === ParseModes.VERSE ? null : resultNode;
	 ...
	 * @returns
	 */
	private applyNodeProcessors(nodes: IParsedNode[], context: ParseContext): IParsedNode[] {
		...
		const processor: NodeProcessorFn = (node, context) => {
	 ...
	 * @param context
	 */
	private processOneFigure(node: IParsedNode, context: ParseContext): IFigureInfo {
	 ...
	 * @returns
	 */
	private processCaptionNode(node: IParsedNode, context: ParseContext): IParsedNode {
		if (!node) {
			return node;
		}
		...
		if (node.text) {
			...
			const newText = mdUtils.translateString(node.text.replace(INDEX_RE, ind.toString()));
	 ...
	 * @param context
	 */
	private updateLinksInNode(node: IParsedNode, context: ParseContext): void {
		const children = node.children;
	...
	private replaceLinkDefs(
		node: IParsedNode,
		context: ParseContext
	): IParsedNode | IParsedNode[] {
		const parts: IParsedNode[] = [];
		const linkRE = /\[([^\]]+)\]/g;
	 ...
	 *********************************************************/
	private processTextChildren(node: ParsedNode, context: ParseContext): ParsedNode {
		if (!node) {
			return node;
		}
	...
	private isInlineMLNode(node: IParsedNode): boolean {
