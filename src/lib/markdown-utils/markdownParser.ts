...
class MarkdownParser implements IMarkdownParser {
	...
	private processIdLinks(
		...
		if (node.type as string === MLNODE_TYPES.LINK as string && node.target) {
	...
	private tryProcessFigure(node: IMLParsedNode, context: MLParseContext): void {
		if (node.type as string === MLNODE_TYPES.FIGURE as string) {
	...
	private parsedNodeToMLNode(
		...
		let currentLine: IMLParsedNode =
			parseMode as string === MLParseModes.VERSE as string ? null : resultNode;
	...
	private processOneFigure(
		...
		const config = context.metaData.captions[MLNODE_TYPES.FIGURE];
		...
		const captionNodes = node.children?.filter(
				(c) => c.type as string === MLNODE_TYPES.FIGCAPTION as string
			),
	...
	private updateLinksInNode(
		...
		for (const child of children) {
			if (child.type as string === MLNODE_TYPES.TEXT as string) {
	...
	private processTextChildren(
		...
		const processText =
			parseMode as string === MLParseModes.VERSE as string
		...
		for (let i = 0, len = children.length; i < len; ++i) {
			...
			if (child.type as string === ASTNODE_TYPES.TEXT as string) {
